 ...
 *
 */
public class ContainerUnitCredit implements IUnitCredit {

    private static final Logger log = LoggerFactory.getLogger(ContainerUnitCredit.class);
    ...
    public void provision(IAccount acc, int productInstanceId, Date startDate, boolean verifyOnly,
        ...
        String specsToProvision = getPropertyFromConfig("UnitCreditSpecIdsToProvision");
        String relativeValue = getPropertyFromConfig("UnitCreditValuePercentages");

        String[] specIds = specsToProvision.split(",");
        String[] relativeValues = relativeValue.split(",");
        int sum = 0;
        int cnt = 0;
        for (String val : relativeValues) {
            sum += Utils.getStringAsInt(val, 0);
            cnt++;
        }
        if (sum != 100 || cnt != specIds.length) {
            throw new Exception("Invalid configuration of ContainerUnitCredit -- " + sum + " " + cnt + " " + specIds.length);
        }

        List<IUnitCredit> ucList = new ArrayList<>();
        int i = 0;
        for (String spec : specIds) {

            int actualUCSpecId = Utils.getStringAsInt(spec, 0);
            int percent = Utils.getStringAsInt(relativeValues[i], 0);
            ...
            IUnitCredit uc = UnitCreditManager.provisionUnitCredit(
                    ...
                    verifyOnly,
                    extTxid,
                    posCentsPaidEach * percent / 100,
                    posCentsDiscountEach * percent / 100,
                    saleLineId,
                    info);
        ...
        if (getBooleanPropertyFromConfig("SendConsolidatedCLUnitCreditEvent")) {
            EventHelper.sendContainerUnitCreditEvent(ucList, "CL_UC", getPropertyFromConfig("ProvisionedEventSubType"));
            EventHelper.sendContainerUnitCreditEvent(ucList, "CL_UC", getPropertyFromConfig("ProvisionedEventSubType1"));
            EventHelper.sendContainerUnitCreditEvent(ucList, "CL_UC", getPropertyFromConfig("ProvisionedEventSubType2"));
        }
