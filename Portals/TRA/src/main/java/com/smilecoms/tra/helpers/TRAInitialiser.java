 ...
 */
package com.smilecoms.tra.helpers;
...
@WebServlet(name = "TRAInitialiser", urlPatterns = {"/TRAInitialiser"})
public class TRAInitialiser extends HttpServlet  {
    
    private static final Logger log = LoggerFactory.getLogger(TRAInitialiser.class);
    private static ScheduledFuture runner1 = null;
    
    @Override
    public void init() {
        runner1 = Async.scheduleAtFixedRate(new SmileBaseRunnable("TRA.Initialiser") {
            @Override
            public void run() {
                trigger();
            }
    ...
    private void trigger() {
        try {
            WebSite website = new WebSite("TRA", "http",getListeningPort(), "/TRA/");
            website.setWeight(5);
            website.setTestData("index.html");
            website.setTestResponseCriteria("OK");
