 ...
 * @author paul
 */
@Path("radiusauthentication")
public class RadiusAuthenticationResource extends Resource {

    private static final Logger log = LoggerFactory.getLogger(RadiusAuthenticationResource.class);
    @Context
    ...
    @JsonIgnore
    public String authenticate(@QueryParam("u") String userName, @QueryParam("m") String mac, @QueryParam("ip") String ip) {
        start(request);
        try {
            ...
            throw new SRAException(Status.UNAUTHORIZED);
        } catch (Exception ex) {
