...
public abstract class SIPTransactionStack implements
     ...
     * http://tools.ietf.org/html/rfc5626#section-4.4.1. a value < 0 means that
     * the RFC 5626 will not be triggered, as a default we don't enable it not
     * to change existing apps behavior.
     */
...
//        this.timer = new DefaultTimer();
    ...
    public SIPDialog createDialog(SIPTransaction transaction) {
        ...
        if (transaction instanceof SIPClientTransaction) {
            ...
            if (this.earlyDialogTable.get(dialogId) != null) {
                ...
                retval = new SIPDialog(transaction); // Our dialogues are created here
                this.earlyDialogTable.put(dialogId, retval);
                if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
    ...
    public void removeDialog(SIPDialog dialog) {
        ...
        if (earlyId != null) {
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("removeDialog " + dialog + "/" + earlyId + " earlyDialogTable Sze " + earlyDialogTable.size() + " dialogTable Sze " + dialogTable.size());
                logger.logStackTrace();
            }
        ...
        if (id != null) {
            ...
            if (old == dialog) {
                if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                    logger.logDebug("removeDialog " + id + " dialogTable Sze " + dialogTable.size());
                    logger.logStackTrace();
                }
     ...
     * described above).
     *
     * Due to different app chaining scenarios (ie B2BUA to Proxy), the RFC
     * matching is not enough alone. It maybe several transactions matches the
     * defined criteria. For that reason, some complementary conditions are
     * included. These conditions will be used as a way to prioritize two
     * matched transactions. In case, just one transaction matches the RFC
     * criteria, these additional conditions will be ignored, and the regular
     * logic will be used. Additional conditions are to match notMsg.reqURI with
     * ct.origReq.contact, and prefer transactions with dialogs. See
     * https://github.com/RestComm/jain-sip/issues/60 for more info.
     * Complementary are also used to stop the searching, and return the matched
     * tx. This is because we are iterating the whole client transaction table,
     * which may be big effort.
     *
     * @param notifyMessage
...
//        }
            ...
            if (sipTransaction instanceof SIPServerTransaction) {
                ...
                if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                    logger.logDebug("REMOVED client tx " + removed + " KEY = "
                            + key + " Sze is : " + clientTransactionTable.size());
    ...
    private static long lastLogged = System.currentTimeMillis();
    ...
    private void addTransactionHash(SIPTransaction sipTransaction) {
        ...
        if (sipTransaction instanceof SIPClientTransaction) {
            ...
            if (logger.isLoggingEnabled()) {
                logger.logDebug("PUT " + key + " Sze: " + clientTransactionTable.size() + " Tx:" + sipTransaction.toString());
                if (System.currentTimeMillis() - lastLogged > 10000) {
                    lastLogged = System.currentTimeMillis();
                    for (SIPClientTransaction trans : clientTransactionTable.values()) {
                        logger.logDebug(trans.toString());
                    }
                }
            }
        } else {
    ...
    protected void removeTransactionHash(SIPTransaction sipTransaction) {
        ...
        if (sipTransaction instanceof SIPClientTransaction) {
            String key = sipTransaction.getTransactionId();
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("removing client Tx : " + key + " Sze: " + clientTransactionTable.size() + " Tx:" + sipTransaction.toString());
            }
            removed = clientTransactionTable.remove(key);
    ...
    /**
     * If all calls are occurring on a single TCP socket then the stack would
     * process them in single thread. This property allows immediately after
     * parsing to split the load into many threads which increases the
     * performance significantly. If set to 0 then we just use the old model
     * with single thread.
     *
     * @return
     */
     ...
     *
     * @param sipRequest is the request for which we want to compute the next
     * hop.
     * @throws SipException
     */
...
//            throw new IllegalStateException("Stack has been stopped, no further tasks can be scheduled.");
    ...
    public void addForkedClientTransaction(
        ...
        if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
            logger.logDebug(
                    ...
                    + " sipDialogId= " + clientTransaction.getDefaultDialog().getDialogId() + "Sze: " + forkedClientTransactionTable.size());
        }
    ...
    /**
     * @param deliverTerminatedEventForAck the deliverTerminatedEventForAck to
     * set
     */
...
//        } else 
    ...
    /**
     * Find suitable MessageProcessor and calls it's
     * {@link MessageProcessor#closeReliableConnection(String, int)} method
     * passing peerAddress and peerPort as arguments.
     *
     * @param myAddress - server ip address
     * @param myPort - server port
    ...
    /**
     * @param stackCongestionControlTimeout the stackCongestionControlTimeout to
     * set
     */
