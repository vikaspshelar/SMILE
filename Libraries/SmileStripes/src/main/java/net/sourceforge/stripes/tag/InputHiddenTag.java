...
/**
 * <p>Generates one or more {@literal <input type="hidden" ... />} HTML tags
 * based on the value supplied. The hidden tag assigns the value attribute by
 * scanning in the following order: <ul> <li>for one or more values with the
 * same name in the HttpServletRequest</li> <li>for a field on the ActionBean
 * with the same name (if a bean instance is present)</li> <li>by collapsing the
 * body content to a String, if a body is present</li> <li>referring to the
 * result of the EL expression contained in the value attribute of the tag.</li>
 * </ul> </p>
 *
 * <p>The result of this scan can produce either a Collection, an Array or any
...
public class InputHiddenTag extends InputTagSupport implements BodyTag {
    ...
    private boolean nginit; // PCB SMILE
    ...
    public int doEndInputTag() throws JspException {
        ...
        if (valueOrValues == null) {
            ...
            for (int i = 0; i < len; ++i) {
                ...
                // PCB Smile start
                if (nginit) {
                    getAttributes().put("ng-init", getName() + "='" + format(value) + "'");
                }
                // PCB Smile end
            ...
            // PCB Smile start
            if (nginit) {
                getAttributes().put("ng-init", getName() + "='" + format(valueOrValues) + "'");
            }
            // PCB Smile end
        ...
        // PCB Smile start
        getAttributes().remove("ng-init");
        // PCB Smile end

        return EVAL_PAGE;
    }

    public boolean isNginit() {
        return nginit;
    }

    public void setNginit(boolean nginit) {
        this.nginit = nginit;
    }
}
