...
public class DispatcherHelper {
    ...
    private static final Logger smilelog = Logger.getLogger("com.smilecoms.stripes");
    ...
    public static Resolution invokeEventHandler(ExecutionContext ctx) throws Exception {
        ...
        return ctx.wrap( new Interceptor() {
            public Resolution intercept(ExecutionContext ctx) throws Exception {
                              ...
                              // Smile PCB START - Log our stats before calling action bean
                String user = ctx.getActionBean().getContext().getRequest().getRemoteUser();
                String userStr = user == null ? "" : ("[" + user + "] ");
                String actionBean = handler.getDeclaringClass().getName().replace("com.smilecoms.", "").replace(".action.", ".").replace("ActionBean", "") + "." + handler.getName();
                long start = System.currentTimeMillis();
                if (smilelog.isLoggable(Level.FINEST)) {
                    smilelog.finest(userStr + "Stripes Dispatcher calling: " + handler.getDeclaringClass().getName() + ":" + handler.getName());
                }
                // Smile PCB END - Log our stats before calling action bean
                ...
                try {
                    ...
                    try {
                        // Smile PCB START - Log our stats before calling action bean
                        if (!smilelog.isLoggable(Level.FINEST) && smilelog.isLoggable(Level.FINE)) {
                            smilelog.fine(userStr + "Stripes Dispatcher called : " + handler.getDeclaringClass().getName() + ":" + handler.getName() + " (" + (System.currentTimeMillis() - start) + "ms)");
                        }
                        BaseUtils.addStatisticSample(actionBean, BaseUtils.STATISTIC_TYPE.latency, System.currentTimeMillis() - start);
                    } catch (Throwable e) {
                        log.warn("Error: ", e);
                    }
                }
