...
public class ManagerReaderImpl implements ManagerReader {
     ...
     *
     * @see
     * org.asteriskjava.manager.internal.Dispatcher#dispatchEvent(ManagerEvent)
     * @see
     * org.asteriskjava.manager.internal.Dispatcher#dispatchResponse(ManagerResponse)
     */
    public void run() {
        ...
        try {
            ...
            while (!this.die && (line = socket.readLine()) != null) {
                ...
                if (line.isEmpty() && skipLine) {
                    // End of message so turn skipping off
                    skipLine = false;
                    continue;
                }
                if (skipLine) {
                ...
                if (line.length() > 0) {
                    ...
                    if (delimiterIndex > 0 && line.length() > delimiterIndex + delimiterLength) {
                        ...
                        // PCB SMILE performance improvement to ignore events we dont need
                        if (name.equals("event") && (value.equalsIgnoreCase("rtcpsent") || value.equalsIgnoreCase("varset") || value.equalsIgnoreCase("newexten") || value.equalsIgnoreCase("localbridge"))) {
                            skipLine = true;
                ...
                if (line.isEmpty()) {
                    String e;
                    // PCB - Smile. We only need newstate events so ignore all else for performance reasons
                    if (buffer.get("event") != null) {
                        // TODO tracing
                        ...
                        if (response != null) {
                            logger.debug("Dispatching response");
